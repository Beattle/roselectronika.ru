...
class SelectTemplateStep extends CSelectTemplateWizardStep
{
	...
	function OnPostForm()
	{
		...
		{
			$arTemplates = array("ms_shop_v3_adapt", "ms_shop_v3_adapt_vertical");
			...
			if (in_array($templateID,  array("ms_shop_v3_adapt", "ms_shop_v3_adapt_vertical")))
				$wizard->SetVar("templateID", "ms_shop_v3_adapt");
			else
				$wizard->SetVar("templateID", "ms_shop");
		}
	}
	...
	function ShowStep()
	{
		...
		if (in_array("ms_shop_v3_adapt", array_keys($arTemplates)))
		{
			$arTemplateOrder[] = "ms_shop_v3_adapt";
		}

		$defaultTemplateID = COption::GetOptionString("main", "wizard_template_id", "ms_shop_v3_adapt", $wizard->GetVar("siteID"));
		if (!in_array($defaultTemplateID, array("ms_shop_v3_adapt", "ms_shop_v3_adapt_vertical"))) $defaultTemplateID = "ms_shop_v3_adapt";
		$wizard->SetDefaultVar("wizTemplateID", $defaultTemplateID);

		$arTemplateInfo = array(
			"ms_shop_v3_adapt" => array(
				"NAME" => GetMessage("WIZ_TEMPLATE_ADAPT"),
				"DESCRIPTION" => GetMessage("WIZ_TEMPLATE_ADAPT_DESCRIPTION"),
				"PREVIEW" => $wizard->GetPath()."/site/templates/ms_shop_v3_adapt/lang/".LANGUAGE_ID."/preview_horizontal.png",
				"SCREENSHOT" => $wizard->GetPath()."/site/templates/ms_shop_v3_adapt/lang/".LANGUAGE_ID."/screen_horizontal.png",
			),
		);


	//	$this->content .= "<input type='hidden' value='ms_shop' name='templateID' id='templateID'>";//$this->ShowInputField('hidden', 'templateID', array("id" => "templateID", "value" => "ms_shop"));
...
class SiteSettingsStep extends CSiteSettingsWizardStep
{
	function InitStep()
	{
		...
		if(COption::GetOptionString("ms_shop", "wizard_installed", "N", $siteID) == "Y" && !WIZARD_INSTALL_DEMO_DATA)
		...
		if ($wizard->GetVar("templateID") == "ms_shop")
		{
			$arSocNetsValues["facebook"] = COption::GetOptionString("ms_shop", "shopFacebook", GetMessage("WIZ_SHOP_FACEBOOK_DEF"), $siteID);
			$arSocNetsValues["twitter"] = COption::GetOptionString("ms_shop", "shopTwitter", GetMessage("WIZ_SHOP_TWITTER_DEF"), $siteID);
			$arSocNetsValues["vk"] = COption::GetOptionString("ms_shop", "shopVk", GetMessage("WIZ_SHOP_VK_DEF"), $siteID);
			$arSocNetsValues["google"] = COption::GetOptionString("ms_shop", "shopGooglePlus", GetMessage("WIZ_SHOP_GOOGLE_PLUS_DEF"), $siteID);
		}
		else
		{
		...
		$wizard->SetDefaultVars(
			Array(
				...
				"shopEmail" => COption::GetOptionString("ms_shop", "shopEmail", "sale@".$_SERVER["SERVER_NAME"], $siteID),
				...
				"installms_shopApp" => COption::GetOptionString("ms_shop", "installms_shopApp", "Y", $siteID),	
			)
		);
...
//install Demo data		
		...
		if (file_exists($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/ms_shopapp") && LANGUAGE_ID == "ru")
			$this->content .= '
			<div class="wizard-input-form-block">
				'.$this->ShowCheckboxField("installms_shopApp", "Y", array("id"=>"installms_shopApp")).
				' <label for="installms_shopApp">'.GetMessage("wiz_install_ms_shopapp").'</label>
			</div>';
...
class CatalogSettings extends CWizardStep
{
	function InitStep()
	{
		...
		$wizard->SetDefaultVars(
			Array(
				"catalogSubscribe" => (isset($arSubscribe[$siteID])) ? ($arSubscribe[$siteID]['use'] == "Y" ? "Y" : false) : "Y",//COption::GetOptionString("ms_shop", "catalogSubscribe", "Y", $siteID),
				"catalogView" => COption::GetOptionString("ms_shop", "catalogView", "list", $siteID),
...
class ShopSettings extends CWizardStep
{
	function InitStep()
	{
		...
		$wizard->SetDefaultVars(
			Array(
				"shopLocalization" => COption::GetOptionString("ms_shop", "shopLocalization", "ru", $siteID),
				"shopEmail" => COption::GetOptionString("ms_shop", "shopEmail", "sale@".$_SERVER["SERVER_NAME"], $siteID),
				"shopOfName" => COption::GetOptionString("ms_shop", "shopOfName", GetMessage("WIZ_SHOP_OF_NAME_DEF"), $siteID),
				"shopLocation" => COption::GetOptionString("ms_shop", "shopLocation", GetMessage("WIZ_SHOP_LOCATION_DEF"), $siteID),
				//"shopZip" => 101000,
				"shopAdr" => COption::GetOptionString("ms_shop", "shopAdr", GetMessage("WIZ_SHOP_ADR_DEF"), $siteID),
				"shopINN" => COption::GetOptionString("ms_shop", "shopINN", "1234567890", $siteID),
				"shopKPP" => COption::GetOptionString("ms_shop", "shopKPP", "123456789", $siteID),
				"shopNS" => COption::GetOptionString("ms_shop", "shopNS", "0000 0000 0000 0000 0000", $siteID),
				"shopBANK" => COption::GetOptionString("ms_shop", "shopBANK", GetMessage("WIZ_SHOP_BANK_DEF"), $siteID),
				"shopBANKREKV" => COption::GetOptionString("ms_shop", "shopBANKREKV", GetMessage("WIZ_SHOP_BANKREKV_DEF"), $siteID),
				"shopKS" => COption::GetOptionString("ms_shop", "shopKS", "30101 810 4 0000 0000225", $siteID),
				"siteStamp" => COption::GetOptionString("ms_shop", "siteStamp", $siteStamp, $siteID),

				//"shopCompany_ua" => COption::GetOptionString("ms_shop", "shopCompany_ua", "", $siteID),
				"shopOfName_ua" => COption::GetOptionString("ms_shop", "shopOfName_ua", GetMessage("WIZ_SHOP_OF_NAME_DEF_UA"), $siteID),
				"shopLocation_ua" => COption::GetOptionString("ms_shop", "shopLocation_ua", GetMessage("WIZ_SHOP_LOCATION_DEF_UA"), $siteID),
				"shopAdr_ua" => COption::GetOptionString("ms_shop", "shopAdr_ua", GetMessage("WIZ_SHOP_ADR_DEF_UA"), $siteID),
				"shopEGRPU_ua" =>  COption::GetOptionString("ms_shop", "shopEGRPU_ua", "", $siteID),
				"shopINN_ua" =>  COption::GetOptionString("ms_shop", "shopINN_ua", "", $siteID),
				"shopNDS_ua" =>  COption::GetOptionString("ms_shop", "shopNDS_ua", "", $siteID),
				"shopNS_ua" =>  COption::GetOptionString("ms_shop", "shopNS_ua", "", $siteID),
				"shopBank_ua" =>  COption::GetOptionString("ms_shop", "shopBank_ua", "", $siteID),
				"shopMFO_ua" =>  COption::GetOptionString("ms_shop", "shopMFO_ua", "", $siteID),
				"shopPlace_ua" =>  COption::GetOptionString("ms_shop", "shopPlace_ua", "", $siteID),
				"shopFIO_ua" =>  COption::GetOptionString("ms_shop", "shopFIO_ua", "", $siteID),
				"shopTax_ua" =>  COption::GetOptionString("ms_shop", "shopTax_ua", "", $siteID),

				"installPriceBASE" => COption::GetOptionString("ms_shop", "installPriceBASE", "Y", $siteID),
			)
		);
	}
	...
	function InitStep()
	{
		...
		if ($shopLocalization == "ua")
			...
			$wizard->SetDefaultVars(
				Array(
					"personType" => Array(
						"fiz" =>  COption::GetOptionString("ms_shop", "personTypeFiz", "Y", $siteID),
						"ur" => COption::GetOptionString("ms_shop", "personTypeUr", "Y", $siteID),
					)
				)
			);
	}
...
===*/
	...
	function ShowStep()
	{
		...
		if (
			...
			LANGUAGE_ID == "ru" &&
			(
				COption::GetOptionString("ms_shop", "wizard_installed", "N", $siteID) != "Y"
			...
			<div class="wizard-input-form-block">
				...
				<div>
					<div class="wizard-input-form-field wizard-input-form-field-checkbox">';
						...
						if(COption::GetOptionString("ms_shop", "wizard_installed", "N", $siteID) != "Y")
						{
...
class DataInstallStep extends CDataInstallWizardStep
{
	...
	{
		if($_SESSION["BX_ms_shop_LOCATION"] == "Y")
...
class FinishStep extends CFinishWizardStep
{
	...
	//	$this->content .= "<br clear=\"all\"><a href=\"/bitrix/admin/wizard_install.php?lang=".LANGUAGE_ID."&site_id=".$siteID."&wizardName=bitrix:ms_shop.mobile&".bitrix_sessid_get()."\" class=\"button-next\"><span id=\"next-button-caption\">".GetMessage("wizard_store_mobile")."</span></a><br>";
