...
if ($REQUEST_METHOD == "POST" && strlen($Update) > 0 && $isAdmin && check_bitrix_sessid()) {
	if (!$message) {
        ...
        if($rr) {
            if (strlen($apply) <= 0) {
    			LocalRedirect("/bitrix/admin/step2use_redirects_list.php?lang=" . LANG . "&filter_site_id=" . UrlEncode($site_id) . "&" . GetFilterParams("filter_", false));
    		};
    	} else {
    		$message = new CAdminMessage(array(
...
// find redirect by ID
$arResultList = S2uRedirectsRulesDB::GetList(array("ID" => trim($ID)/* ,'ADRESS'=>' ','DATE_TIME_CREATE'=>' ','ACTIVE'=>' ', 'COMMENT'=>' ' */));
...
<form method="POST" action="<?= $APPLICATION->GetCurPage() ?>" name="form1">
	...
	$tabControl->Buttons(
        array(
            "disabled" => !$isAdmin,
			"back_url" => "/bitrix/admin/step2use_redirects_list.php?lang=" . LANG . "&filter_site_id=" . UrlEncode($site_id) . "&" . GetFilterParams("filter_", false)
        )
    );
