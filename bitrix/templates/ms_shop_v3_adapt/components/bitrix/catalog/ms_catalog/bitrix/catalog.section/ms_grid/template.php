...
<div id="catalog" class="bx_catalog_grid col<? echo $arParams['LINE_ELEMENT_COUNT']; ?> <? echo $templateData['TEMPLATE_CLASS']; ?>">
	<?
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
			...
			<a id="<? echo $arItemIDs['PICT']; ?>"
				...
				?>
					<span
						...
						style="display:<? echo (0 < $arItem['MIN_PRICE']['DISCOUNT_DIFF_PERCENT'] ? '' : 'none'); ?>;">-<? echo $arItem['MIN_PRICE']['DISCOUNT_DIFF_PERCENT']; ?>%</span>
				<?
				}			
				?>
				</span>	
				...
				<?if (!empty($arItem['MIN_PRICE']) && $arItem['MIN_PRICE']['VALUE'] != 0) {?>
					...
					<?} else {?>
						...
						<?} else {?>
                            ...
                            <?if($boolQP):?><span class="old-qp">(Старая цена)</span> <?endif;?>
						<?}?>
					<?}?>
		...
		?>
			...
			?>
				...
				<!--<div class="bx_catalog_item_controls">
					...
					foreach ($arItem['DISPLAY_PROPERTIES'] as $arOneProp)
					{
						if($arOneProp["CODE"] !== "NEWPRODUCT" && $arOneProp["CODE"] !== "SALELEADER" && $arOneProp["CODE"] !== "SPECIALOFFER") {
						?><li><? echo $arOneProp['NAME']; ?>: <b><?
							echo (
		...
		else
		{
			...
			<div class="bx_catalog_item_controls touch">
				<a class="btn bt2" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>"><?
				echo ('' != $arParams['MESS_BTN_DETAIL'] ? $arParams['MESS_BTN_DETAIL'] : GetMessage('CT_BCS_TPL_MESS_BTN_DETAIL'));
				?></a>
			</div>
