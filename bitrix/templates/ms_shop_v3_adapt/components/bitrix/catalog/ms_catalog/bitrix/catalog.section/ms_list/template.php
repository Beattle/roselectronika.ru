							...
							<ul id="#ITEM#_prop_<? echo $arProp['ID']; ?>_list" style="width: <? echo $strWidth; ?>;">
								<?foreach ($arProp['VALUES'] as $arOneValue) {?>
									<li data-treevalue="<? echo $arProp['ID'].'_'.$arOneValue['ID']; ?>" data-onevalue="<? echo $arOneValue['ID']; ?>" style="width: <? echo $strOneWidth; ?>;">
										<i></i>
										<span class="cnt"><? echo htmlspecialcharsex($arOneValue['NAME']); ?>
										</span>
									</li>
								<?}?>
							</ul>
...
<!-- Catalog -->
<div id="catalog" class="bx_catalog_list <? echo $templateData['TEMPLATE_CLASS']; ?>">
	<?
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		?>
		<div data-pid="<?=$arItem['ID']?>" class="clearfix <? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?> p_id_<?=$arItem['ID']?> <?=$stock?>">
				...
				<div class="col-2">
					<div class="p-name">
                        <a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $arItem['NAME']; ?>">
                            <?=$arItem["PROPERTIES"]["MODEL"]["VALUE"]?><br/>
                            <? echo $arItem['NAME']; ?>
                        </a>
                    </div>
						...
						<div class="preview-text-in">
							...
							<?if (isset($arItem['DISPLAY_PROPERTIES']) && !empty($arItem['DISPLAY_PROPERTIES'])) {?>
								<div class="item_props">
									<a class="props_show" href="#"><?=GetMessage('CT_BCS_CATALOG_PROPS')?></a>
									<ul>
										<?foreach ($arItem['DISPLAY_PROPERTIES'] as $arOneProp) {?>
										<li>
											<? echo $arOneProp['NAME']; ?>:
				...
				<div class="col-3">
					...
					<?if ('Y' == $arParams['SHOW_MAX_QUANTITY']) {?>
						<?if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS'])) {?>					
							<?if($arItem["CATALOG_QUANTITY"] > 0) {?>
								<div class="quantity">
									<i class="yes"></i>
									<?=GetMessage("QUANTITY_AVAILABLE")?> (<?=$arItem["CATALOG_QUANTITY"]?>)
								</div>
							<?} else {?>
							<div class="quantity">
								<i></i>
					...
					<div class="bx_catalog_item_price">
						<div id="<? echo $arItemIDs['PRICE']; ?>" class="bx_price">
							<?if (!empty($arItem['MIN_PRICE']) && $arItem['MIN_PRICE']['VALUE'] !=0) {?>
								...
								<?} else {?>
									...
									<?} else {?>
                                        <?$boolQP = (int)$arItem["CATALOG_QUANTITY"] === 0;?>
										<?echo $arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE'];?>                                     <?if($boolQP):?>
                                        <span class="old-qp">(Старая цена)</span>
                                        <?endif;?>
									<?}?>
								<?}?>
								...
								<?}?>
							<?} elseif($arItem['MIN_PRICE']['VALUE'] == 0) echo 'На заказ'?>
						</div>
					</div>
					<!-- End Price -->
						...
						<div class="bx_catalog_item_controls">
							<?if ($arItem['CAN_BUY']) {?>
								...
								<div class="bx_catalog_item_controls_blocktwo">
									<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="btn bt1 bx_cart" href="javascript:void(0)" rel="nofollow"><?
									echo ('' != $arParams['MESS_BTN_BUY'] ? $arParams['MESS_BTN_BUY'] : GetMessage('CT_BCS_TPL_MESS_BTN_ADD_TO_BASKET'));
									?></a>
								</div>
								
							<?} else {?>	
<!--
								<div class="bx_catalog_item_controls_blocktwo" style="opacity:0.5;">
									<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="bt2" rel="nofollow">
										<?=GetMessage("QUANTITY_NOT_AVAILABLE")?>
									</a>
								</div>
-->							
								...
								?>
								<div class="bx_catalog_item_controls_blocktwo">
									<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="bx_cart btn none" href="javascript:void(0)" rel="nofollow">
										<?
						...
						<div class="link compare checkbox">
							<input type="checkbox" id="prc_<?=$arItem['ID']?>" value="?action=ADD_TO_COMPARE_LIST&amp;id=<?=$arItem['ID']?>">
							<label for="prc_<?=$arResult['ID']?>">
								<span class="compare_label"><? echo GetMessageJS('CT_BCS_TPL_MESS_BTN_COMPARE'); ?></span>
								<span class="compared_label"><? echo GetMessageJS('CT_BCS_TPL_MESS_BTN_COMPARED'); ?></span>
							</label>
						</div>
					...
					<?} else {?>
						...
						<?} else {?>
							<div class="bx_catalog_item_controls no_touch">
								<a class="bt2" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>">
						...
						<div class="link compare checkbox">
							<input type="checkbox" id="prc_<?=$arItem['ID']?>" value="?action=ADD_TO_COMPARE_LIST&amp;id=<?=$arItem['ID']?>">
							<label for="prc_<?=$arItem['ID']?>"><? echo GetMessageJS('CT_BCS_TPL_MESS_BTN_COMPARE'); ?></label>
						</div>			
		...
		<script type="text/javascript">
		var ItemJsCompare_<?=$arItem['ID']?> = {
			ID:"<?=$arItem['ID']?>",
			productName:"<? echo $arItem['NAME']; ?>",
			productImg:"<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>"
		};
		</script>	
...
<script type="text/javascript">
BX.message({
	MESS_BTN_BUY: '<? echo ('' != $arParams['MESS_BTN_BUY'] ? CUtil::JSEscape($arParams['MESS_BTN_BUY']) : GetMessageJS('CT_BCS_TPL_MESS_BTN_ADD_TO_BASKET')); ?>',
	...
	var showFilter = true,
		productsCount = <?=count($arResult['ITEMS'])?>;
</script>
<?
