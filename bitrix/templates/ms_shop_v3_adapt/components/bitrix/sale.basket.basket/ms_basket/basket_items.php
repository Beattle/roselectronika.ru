...
<div id="basket_items_list">
			...
			<thead>
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
						...
						if ($arHeader["id"] == "NAME"):
						?>
							<td class="itemphoto"><?=getColumnName($arHeader)?></td>
							<td class="item"></td>
						<?
						elseif ($arHeader["id"] == "PRICE"):
						?>
							<td class="price" id="col_<?=getColumnId($arHeader)?>"><?=getColumnName($arHeader)?></td>
						<?
						elseif ($arHeader["id"] == "QUANTITY"):
						?>
							<td class="quantity" id="col_<?=getColumnId($arHeader)?>"><?=getColumnName($arHeader)?></td>
						<?
						elseif ($arHeader["id"] == "DISCOUNT"):
						?>
							<td class="discount" id="col_<?=getColumnId($arHeader)?>"><?=getColumnName($arHeader)?></td>
						<?
						elseif ($arHeader["id"] == "WEIGHT"):
						?>
							<td class="weight" id="col_<?=getColumnId($arHeader)?>"><?=getColumnName($arHeader)?></td>
						<?
						else:
						?>
							<td class="custom" id="col_<?=getColumnId($arHeader)?>"><?=getColumnName($arHeader)?></td>
						<?
						endif;
						?>
					<?
						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							?>
								<td class="itemphoto">
									...
									<div class="bx_ordercart_photo_container">
										...
										<a class="delete_from_basket" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>" title="<?=GetMessage("SALE_DELETE")?>"></a>
									</div>
									<?
								...
								<td class="item">
									...
									<?
									if ($bPropsColumn && count($arItem["PROPS"])):
									?>
							...
							?>
								<td class="quantity custom">
									<div class="cart-counter can_change">
										...
										{
											?>
												<div class="measure_text"><?=$arItem["MEASURE_TEXT"]?></div>
											<?
										}
										?>
	...
	<div class="bx_ordercart_order_pay">
		...
		<div class="bx_ordercart_order_pay_left">
			<div class="bx_ordercart_coupon">
				...
				?>
					...
					<input type="text" id="coupon" name="COUPON" value="<?=$arResult["COUPON"]?>" onchange="enterCoupon();" size="21" class="input_text_style <?=$couponClass?>">
				<?else:?>
					&nbsp;
				<?endif;?>
			</div>
		...
		<div class="bx_ordercart_order_pay_center">
			...
			<a href="javascript:void(0)" onclick="checkOut();" class="checkout bt1 bt-big"><?=GetMessage("SALE_ORDER")?></a>
		</div>
	</div>
</div>
<?
