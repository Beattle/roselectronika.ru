		...
		*/
		...
		if (this.canBuy)
		{
			if(inCart(this.offers[index]['ID'])){
				inCart();
				$('#'+this['visual']['ID']).parent().removeClass('no_stock');
			}
			else{
				$(this.obBuyBtn)
					.removeClass('bt3 bt2 none')
					.addClass('bx_bt_button bt1')
		...
		else
		{
			...
			$('#'+this['visual']['ID']).parent().addClass('no_stock');
			/*
				BX.removeClass(this.obBuyBtn, 'bx_bt_button');
				BX.addClass(this.obBuyBtn, 'bx_bt_button_type_2');
				this.obBuyBtn.innerHTML = BX.message('MESS_NOT_AVAILABLE');
		...
		if(!inCart(this.offers[index]['ID'])){
			$('#'+this['visual']['ID']).parent().removeClass('incart');
		}
		...
		if (this.showQuantity)
		{
			...
			if(this.maxQuantity > 0){
				$('#'+this['visual']['ID']).find('.quantity').html('<i class="yes"></i> '+BX.message('QUANTITY_AVAILABLE').replace('#',this.maxQuantity));
			}
			else{
				$('#'+this['visual']['ID']).find('.quantity').html('<i></i> '+BX.message('MESS_NOT_AVAILABLE'));
			}
		}
	}
};
...
window.JCCatalogSection.prototype.Basket = function()
{
if(!$('#'+this.visual.ID).parent().hasClass('incart')){
	var contentBasketProps = '';
	if (!this.canBuy)
	{
		return;
...
window.JCCatalogSection.prototype.BasketResult = function(arResult)
{
	...
	{
		strContent = (!!arResult.MESSAGE ? arResult.MESSAGE : BX.message('BASKET_UNKNOWN_ERROR'));
		buttons = "<p><a href='javascript:void(0);' onclick='$.fancybox.close()' class='btn bt1 bt-medium'>"+BX.message('BTN_MESSAGE_CLOSE')+"</a></p>";
	}
