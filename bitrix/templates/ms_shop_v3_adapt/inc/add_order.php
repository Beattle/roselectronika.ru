...
CModule::IncludeModule("sale");
  $EVENT_TYPE = 'NEW_ORDER'; // тип почтового шаблона   
			...
			$arMailFields['PERSON_TYPE'] = $_REQUEST['person_type'];			
	...
	////////////////////////////////////////////////	82
		...
		if ($_REQUEST["product_name100"] == "undefined"){
			...
			if(!empty($arMailFields))
			{
				...
				$PROP = array();
				$PROP["FIO"] = $arMailFields['LAST_NAME'];
				$PROP["MAIL"] = $arMailFields['EMAIL'];
				$PROP["PHONE"] = $arMailFields['NAME'];
				$PROP["CITY"] = $arMailFields['CITY'];
				$PROP["PERSON_TYPE"] = $arMailFields['PERSON_TYPE'];
				$PROP["PRICE_ALL"] = $_REQUEST['all_summ'];
				$PROP["TOVAR"] = '';
				$PROP["ORDER_ID"] = '';
				
				foreach($arMailFields as $ar_fild_key => $ar_fild)
				{
					$pos = strripos($ar_fild_key, 'PRODUCT');
					if($pos !== false)
					{
						$index = str_replace('PRODUCT', '', $ar_fild_key);
						if(empty($arMailFields['PRODUCT'.$index]) || $arMailFields['PRODUCT'.$index] == " ") continue;
						
						if($index == 1)
						{
							$NUMBER = $arMailFields['NUMBER'];
						}
						else
						{
							$NUMBER = $arMailFields['NUMBER'.$index];
						}
						$PROP["TOVAR"] .= $NUMBER.' '.$arMailFields['PRODUCT'.$index].' '.$arMailFields['LINK'.$index].' '.'Количество: '.$arMailFields['COUNT'.$index].'<br>
						';
					}
				};
				...
				$res = CIBlockElement::GetList(array("PROPERTY_ORDER_ID" => "DESC"), $arFilter, false, array("nPageSize" => 1), $arSelect);
				if($ob = $res->GetNextElement())
				{
					$ar_fields_order = $ob->GetFields();
					$ar_fields_order["Props"] = $ob->GetProperties();
					$PROP["ORDER_ID"] = intval($ar_fields_order["Props"]["ORDER_ID"]["VALUE"]) + 1;					
				}
				if(empty($PROP["ORDER_ID"])) $PROP["ORDER_ID"] = 170051;
				...
				$arMailFields["ORDER_ID"] = $PROP["ORDER_ID"];//$ID;
			}
