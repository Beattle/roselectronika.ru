	...
	<form name="form_auth" method="post" target="_top" action="<?=SITE_DIR?>auth/<?//=$arResult["AUTH_URL"]?>" class="bx_auth_form">
		...
		<div class="clearfix">
			<?if ($arResult["STORE_PASSWORD"] == "Y"):?>
				<label class="rememberme checkbox fl">
			...
			<noindex>
				<span class="forgotpassword fr" style="padding-left:75px;"><a href="<?=$arParams["AUTH_FORGOT_PASSWORD_URL"] ? $arParams["AUTH_FORGOT_PASSWORD_URL"] : $arResult["AUTH_FORGOT_PASSWORD_URL"]?>" rel="nofollow"><?=GetMessage("AUTH_FORGOT_PASSWORD_2")?></a></span>
			</noindex>
			<?endif?>
		</div>
		
		<br><input type="submit" name="Login" class="btn bt1 bt-medium" value="<?=GetMessage("AUTH_AUTHORIZE")?>" />
		
		<br/>
		<br/>
		<p>
			<a href="<?=SITE_DIR?>auth/?register=yes" rel="nofollow"><b><?=GetMessage("REGISTRATION_TITLE")?></b></a><br>
			<?=GetMessage("AUTH_REGISTRATION_NOTE")?>
		</p>
	</form>
</div>
<script type="text/javascript">
