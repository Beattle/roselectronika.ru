...
if (0 < $arResult['SECTIONS_COUNT'])
{
	if ('LIST' != $arParams['VIEW_MODE'])
	{
		...
		{
            if(false === $arOneSection['PICTURE']){
                $arOrder = array('order'=>'asc',);
                $arGroupBy = false; // array('ID',"NAME","PREVIEW_PICTURE");
                $arSelect = Array("ID", "NAME", "DETAIL_PICTURE","PREVIEW_PICTURE");
                $arFilter = Array('SECTION_ID'=>$arOneSection['ID'], "ACTIVE"=>"Y");
                $res_1prod = CIBlockElement::GetList($arOrder, $arFilter, $arGroupBy, Array("nTopCount"=>1), $arSelect);
                if($ob = $res_1prod->GetNext())
                {
                    $arOneSection['PICTURE'] = CFile::GetFileArray($ob['PREVIEW_PICTURE']);
                }
            }
